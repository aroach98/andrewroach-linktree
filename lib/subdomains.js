...
// Subdomains that must never be published, even though they're attached to a
// project on this team and would otherwise be discovered automatically.
//
// This list is not cosmetic. Private tools are gated by their own auth, but
// listing one here announces that it exists and invites attention it doesn't
// want. Auto-discovery is the right default for a link tree and the wrong
// default for anything private, so private hosts must opt out explicitly.
const HIDDEN_SUBDOMAINS = new Set(
  (process.env.HIDDEN_SUBDOMAINS || "lookup")
    .split(",")
...
function isHidden(host, name) {
  ...
  if (HIDDEN_SUBDOMAINS.has(name.split(".")[0].toLowerCase())) return true; // private
  return false;
}
